...
export default function MedicalSupplies() {
  ...
  const [formData, setFormData] = useState({ id: null, name: '', category: 'Medical Supplies', quantity: 0, unit: 'pcs', restock_level: 10 });
  ...
  const downloadPDF = async () => {
    try {
      const res = await fetch(`/api/inventory?is_equipment=false&category=${activeCategory}&status=${activeStatusFilter}&search=${encodeURIComponent(debouncedSearchQuery)}&paginate=false`);
  ...
  const { data: suggestionData } = useQuery({
    queryKey: ['remarkSuggestions', consumptionSearch],
    queryFn: async () => {
      if (!consumptionSearch) return [];
      const res = await fetch(`/api/inventory/remarks-suggestions?q=${encodeURIComponent(consumptionSearch)}`);
      const data = await res.json();
      return data.data || [];
    },
    enabled: !!consumptionSearch
  });

  useEffect(() => {
    if (suggestionData) {
      setRemarkSuggestions(suggestionData);
      setShowRemarkSuggestions(suggestionData.length > 0);
    } else {
  ...
  const { data: invData, isLoading: loadingInv, refetch: refetchInventory } = useQuery({
    queryKey: ['inventory', activeCategory, false, inventoryPage, debouncedSearchQuery, activeStatusFilter],
    queryFn: async () => {
      const response = await fetch(`/api/inventory?is_equipment=false&page=${inventoryPage}&category=${activeCategory}&status=${activeStatusFilter}&search=${encodeURIComponent(debouncedSearchQuery)}`);
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="w-full">
        <table className="w-full text-left border-collapse">
            ...
            <tr className="bg-[#181822]">
              ...
              <th className="p-4 text-gray-400 font-semibold text-sm border-b border-[#2b2b35]">
                ...
                >
                  ...
                  {activeSection === 'equipment' && (
                    <>
                      <option value="Serviceable" className="bg-[#181822] text-[#34c759]">Serviceable</option>
                      <option value="Unserviceable" className="bg-[#181822] text-[#ff453a]">Unserviceable</option>
                    </>
                  )}
                </select>
              </th>
          ...
          <tbody>
            {loadingInv ? (
              <tr><td colSpan="8" className="p-4 text-center text-gray-400 border-b border-[#1f1f26]">Loading...</td></tr>
            ) : inventory.length === 0 ? (
              <tr><td colSpan="8" className="p-4 text-center text-gray-400 border-b border-[#1f1f26]">No inventory items found.</td></tr>
            ) : inventory.map(item => (
                    ...
                    {activeDropdown === item.id && (
                      <>
                        ...
                        <div className="absolute right-0 mt-2 w-36 bg-[#181822] border border-[#2b2b35] rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col py-1">
                          ...
                          {(item.status === 'Unavailable' || item.status === 'Unserviceable') && (
                            <>
